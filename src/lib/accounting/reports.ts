...
export interface ProfitAndLossResult {
  ...
  expenseByAccount: { code: string; name: string; nameEn: string | null; nameDe: string | null; nameTr: string | null; amount: number }[];
}
...
export async function getProfitAndLoss(workspaceUserId: string, from: Date, to: Date, businessId?: string | null): Promise<ProfitAndLossResult> {
  ...
  for (const account of accounts) {
    ...
    if (account.type === "revenue") {
      ...
      if (amount !== 0) {
        expenseByAccount.push({ code: account.code, name: account.name, nameEn: account.nameEn, nameDe: account.nameDe, nameTr: account.nameTr, amount });
        expenseTotal += amount;
      }
    }
  }
