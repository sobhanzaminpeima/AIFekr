...
export async function runCrmAnalysis(userId: string, onChunk: (text: string) => void, businessId?: string | null): Promise<string> {
  // Insights (the agent's shared memory), analysis runs and auto-created tasks are
  // all business-scoped: business A's strategy notes must never surface in B.
  const snapshot = await buildCrmSnapshot(userId, businessId);
  ...
  for (const line of memLines) {
    ...
    if (m) {
      ...
      await prisma.crmInsight.create({ data: { userId, ...(businessId ? { businessId } : {}), category: m[1], text: m[2].trim(), embedding } });
    }
  }

  // Save the dated report itself -- everything up to (not including) the
  // "future memory" section, same split point the UI already uses to hide
  // that section from the human reader. Without this, the only record of a
  // pipeline analysis was whatever was still on screen until the next page
  // load; there was no way to look back at what last week's read said.
  const displayContent = fullOutput.split(/## نکاتی برای حافظهٔ آینده/)[0].trim();
  if (displayContent) {
    await prisma.crmAnalysisRun.create({ data: { userId, ...(businessId ? { businessId } : {}), content: displayContent } }).catch(() => {});
  }
  ...
  for (const line of highPriorityLines) {
    ...
    if (!title) continue;
    const alreadyExists = await prisma.crmTask.findFirst({ where: { userId, ...(businessId ? { businessId } : {}), title, status: "pending" } });
    if (alreadyExists) continue;
    await prisma.crmTask.create({ data: { userId, ...(businessId ? { businessId } : {}), title, autoGenerated: true } });
  }
