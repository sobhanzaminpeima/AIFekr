...
const SUGGESTIONS = [
  "موجودی نقدی فعلی چقدر است؟",
  "بزرگ‌ترین هزینه این ماه چه بوده؟",
  "آیا کسری نقدینگی در راه است؟",
  "سود خالص این ماه نسبت به ماه قبل چطور بوده؟",
];
...
export default function FinanceAssistantPage() {
  ...
  async function ask(question: string) {
    ...
    try {
      ...
      if (!res.ok || !res.body) throw new Error("خطا در پاسخ‌دهی");
      ...
      setMessages((prev) => {
        const next = [...prev];
        next[next.length - 1] = { role: "assistant", text: e instanceof Error ? e.message : "خطا در پاسخ‌دهی" };
        return next;
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-4 flex flex-col" dir="rtl" style={{ minHeight: "80vh" }}>
      <div className="flex items-center gap-2">
        <Link href="/accounting" className="p-1.5 rounded-lg" style={{ color: "var(--text-secondary)" }}><ArrowRight className="w-4 h-4" /></Link>
        <div>
          <h1 className="text-xl font-bold flex items-center gap-1.5" style={{ color: "var(--text-primary)" }}><Sparkles className="w-5 h-5" style={{ color: "var(--primary)" }} />دستیار هوشمند مالی</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--text-secondary)" }}>فقط بر اساس داده‌های واقعی دفتر کل پاسخ می‌دهد — هرگز عددی نمی‌سازد</p>
        </div>
      </div>

      {anomalies.length > 0 && (
        <div className="rounded-2xl p-4" style={{ background: "rgba(237,161,0,0.08)", border: "1px solid rgba(237,161,0,0.3)" }}>
          <div className="flex items-center gap-1.5 text-sm font-medium mb-2" style={{ color: "#eda100" }}><AlertTriangle className="w-4 h-4" />ناهنجاری‌های شناسایی‌شده</div>
          {anomalies.map((a) => (
            <div key={a.accountCode} className="text-xs" style={{ color: "var(--text-secondary)" }}>
              {a.accountName}: {fmt(a.currentMonthAmount)} این ماه در مقابل میانگین {fmt(a.trailingAverage)} ({a.deviationPercent > 0 ? "+" : ""}{a.deviationPercent}٪)
            </div>
          ))}
        </div>
      )}
      ...
      <div className="flex-1 rounded-2xl p-4 space-y-3 overflow-auto" style={{ background: "var(--surface-1)", border: "1px solid var(--border)", minHeight: "300px" }}>
        {messages.length === 0 ? (
          <div className="space-y-2">
            <p className="text-xs mb-2" style={{ color: "var(--text-muted)" }}>چند نمونه سؤال:</p>
            {SUGGESTIONS.map((s) => (
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="سؤال خود را دربارهٔ وضعیت مالی بپرسید..."
          disabled={loading}
