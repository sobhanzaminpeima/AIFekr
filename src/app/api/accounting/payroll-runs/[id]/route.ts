...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const run = await prisma.accountingPayrollRun.findFirst({ where: { id: params.id, workspaceUserId: ws.workspaceUserId, ...businessFilter(ws) } });
  if (!run) return NextResponse.json({ error: tri(lang, "پیدا نشد", "Not found", "Nicht gefunden") }, { status: 404 });

  const { action } = (await req.json()) as { action?: string };
  try {
    if (action === "approve") {
      const updated = await approvePayrollRun(run.id, user.id, { workspaceUserId: ws.workspaceUserId, businessId: ws.businessId });
      return NextResponse.json({ run: updated });
    }
    if (action === "pay") {
      const updated = await payPayrollRun(run.id, user.id, { workspaceUserId: ws.workspaceUserId, businessId: ws.businessId });
      return NextResponse.json({ run: updated });
    }
