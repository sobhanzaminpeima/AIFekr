...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    if (action === "approve-first-run") return NextResponse.json({ report: await approveFirstRun(params.id, ws.workspaceUserId, ws.businessId) });
    if (action === "pause") return NextResponse.json({ report: await pauseScheduledReport(params.id, ws.workspaceUserId, ws.businessId) });
    if (action === "resume") return NextResponse.json({ report: await resumeScheduledReport(params.id, ws.workspaceUserId, ws.businessId) });
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    await deleteScheduledReport(params.id, ws.workspaceUserId, ws.businessId);
