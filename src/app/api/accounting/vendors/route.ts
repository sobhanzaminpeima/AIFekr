...
export async function GET(req: NextRequest) {
  ...
  const vendors = await prisma.accountingVendor.findMany({ where: { workspaceUserId: ws.workspaceUserId, ...businessFilter(ws) }, orderBy: { name: "asc" } });
...
export async function POST(req: NextRequest) {
  ...
  const vendor = await prisma.accountingVendor.create({ data: { workspaceUserId: ws.workspaceUserId, ...businessFilter(ws), name: name.trim(), phone, email, notes } });
