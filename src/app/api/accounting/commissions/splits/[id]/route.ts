...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const split = await prisma.accountingCommissionSplit.findFirst({
    where: { id: params.id, commissionRecord: { workspaceUserId: ws.workspaceUserId, ...businessFilter(ws) } },
  });
  ...
  try {
    const updated = await payCommissionSplit(params.id, user.id, { workspaceUserId: ws.workspaceUserId, businessId: ws.businessId });
