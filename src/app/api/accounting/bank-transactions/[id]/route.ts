...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const txn = await prisma.accountingBankTransaction.findFirst({ where: { id: params.id, workspaceUserId: ws.workspaceUserId, ...businessFilter(ws) } });
  if (!txn) return NextResponse.json({ error: tri(lang, "پیدا نشد", "Not found", "Nicht gefunden") }, { status: 404 });

  const candidates = await findMatchCandidates(ws.workspaceUserId, params.id, ws.businessId);
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await prisma.accountingBankTransaction.findFirst({ where: { id: params.id, workspaceUserId: ws.workspaceUserId, ...businessFilter(ws) } });
  ...
  try {
    if (body.action === "match") {
      ...
      const txn = await confirmMatch(params.id, body.matchedType, body.matchedId, user.id, { workspaceUserId: ws.workspaceUserId, businessId: ws.businessId });
      return NextResponse.json({ transaction: txn });
    }
    if (body.action === "ignore") {
      const txn = await ignoreTransaction(params.id, user.id, { workspaceUserId: ws.workspaceUserId, businessId: ws.businessId });
      return NextResponse.json({ transaction: txn });
    }
