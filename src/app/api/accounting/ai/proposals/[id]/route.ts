...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    if (action === "approve") return NextResponse.json({ proposal: await approveProposal(params.id, ws.workspaceUserId, user.id, ws.businessId) });
    if (action === "reject") return NextResponse.json({ proposal: await rejectProposal(params.id, ws.workspaceUserId, user.id, ws.businessId) });
