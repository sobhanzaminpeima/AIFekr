...
/** Exports the caller's own CRM contacts or deals as CSV — ?type=contacts (default) | deals. */
export async function GET(req: NextRequest) {
  ...
  if (type === "deals") {
    ...
    const contacts = await prisma.crmContact.findMany({ where: { userId: user.id }, orderBy: { createdAt: "desc" }, take: 5000 });
    csv = toCsv(
      contacts.map((c) => ({
