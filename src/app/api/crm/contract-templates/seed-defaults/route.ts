...
export async function POST(req: NextRequest) {
  ...
  const existing = await prisma.crmContractTemplate.findMany({ where: { userId: ws.workspaceUserId, ...businessFilter(ws) }, select: { name: true } });
  ...
  await prisma.crmContractTemplate.createMany({
    data: toCreate.map((t) => ({ userId: ws.workspaceUserId, ...businessFilter(ws), name: t.name, content: t.content, industrySlug: t.industrySlug })),
  });

  const templates = await prisma.crmContractTemplate.findMany({ where: { userId: ws.workspaceUserId, ...businessFilter(ws) }, orderBy: { createdAt: "desc" } });
