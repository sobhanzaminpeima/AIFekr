...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!ALLOWED_TYPES.has(file.type)) return NextResponse.json({ error: "فقط تصویر JPEG، PNG یا WebP مجاز است" }, { status: 400 });
  if (file.size > MAX_SIZE_BYTES) return NextResponse.json({ error: "حجم تصویر نباید بیشتر از ۸ مگابایت باشد" }, { status: 400 });
